...
V1 梁场基础域、V2 项目与权限基础域、V3 操作审计日志和 V4 梁位工单能力已经冻结。
...
V4 本地冻结版本为 `4.0.0`，目标迁移为 `a4d7e9f2c105`，标签为
`backend-db-v4.0.0`。V3 本地冻结基线为 `c5c260a`，标签为
`backend-db-v3.0.0`；V2 基线提交为 `cdd5d43`。冻结的 V1 基线为 Git 标签
`backend-db-v1.0.0`，其数据契约版本保持 `1.0.0`。

后续设备、告警、时序测点、梁生命周期历史和增量同步仍需先确认业务口径；自动审计接入与梁位工单触发时机由 B 在业务流程中决定，A 只提供底层数据能力。
