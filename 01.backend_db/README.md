...
V1 至 V9 已冻结，V9 冻结标签为 `backend-db-v9.0.0`。V9 统一异常事项记录已完成开发、数据库验证、独立测试和审计，并随冻结提交推送至远端 `dev-db` 分支。
...
-   运输交接事实的录入、读取、筛选、分页、幂等和受控作废
-   运输交接只保存地点、车辆、承运单位和交接人快照，不自动修改梁状态
-   统一异常事项的录入、项目隔离、筛选、分页、外部幂等和受控状态变化
-   异常事项只保存已经判定成立的问题，不计算异常、不接入设备、不发送通知
...
V8 冻结版本为 `8.0.0`，目标迁移为 `f8c2d4e6a709`，标签为 `backend-db-v8.0.0`。V6 本地冻结版本为 `6.0.0`，包含 V5 梁生命周期事件和 V6 梁工序执行记录，目标迁移为 `d4a6f8b0c217`，标签为 `backend-db-v6.0.0`。V4 本地冻结版本为 `4.0.0`，目标迁移为 `a4d7e9f2c105`，标签为 `backend-db-v4.0.0`。V3 本地冻结基线为 `c5c260a`，标签为
`backend-db-v3.0.0`；V2 基线提交为 `cdd5d43`。冻结的 V1 基线为 Git 标签
`backend-db-v1.0.0`，其数据契约版本保持 `1.0.0`。

V9 只建设异常事项数据能力。设备档案、协议接入、测点时序、异常计算、消息通知和通用数据同步仍需单独确认；运输计划、车辆档案和实时轨迹不属于 V8。操作审计写入与业务流程触发时机由 B 决定，A 只提供底层数据能力。
