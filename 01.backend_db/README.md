...
V1 至 V8 已冻结，V8 冻结标签为 `backend-db-v8.0.0`。V8 运输交接记录已完成独立测试和审计，并随冻结提交推送至远端 `dev-db` 分支。
...
后续设备、告警、时序测点和通用数据同步仍需先确认业务口径；运输计划、车辆档案和实时轨迹不属于 V8。操作审计写入与业务流程触发时机由 B 决定，A 只提供底层数据能力。
